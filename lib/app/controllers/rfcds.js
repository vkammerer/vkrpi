...
exports.show = function(req, res) {
	if (req.rfcd.gpio) {
		Gpio.load(req.rfcd.gpio._id, function(err, gpio) {
			if (err) return next(err);
			if (!gpio) return next(new Error('Failed to load gpio ' + req.rfcd.gpio._id));
			thisRfcd = JSON.parse(JSON.stringify(req.rfcd));
			thisRfcd.gpio = gpio;
			res.send(thisRfcd);
		});
	}
